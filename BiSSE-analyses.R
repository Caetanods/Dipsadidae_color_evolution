...
#############################################################################################
## RESULTS FOR ALL THE ALTERNATIVE CATEGORIES:
## Load the results (Comment this part if running the complete analysis)
## Download the file from FigShare.
...
hei.one.A <- to.heidel.diag(mcmc.onerate.A)
hei.two.A <- to.heidel.diag(mcmc.tworate.A)

## hei.one.B <- to.heidel.diag(mcmc.onerate.B)
hei.two.B <- to.heidel.diag(mcmc.tworate.B)
