...
sim.fit.cons <- mclapply(index, FUN = function(x) run.bisse.bound.mle(tree = tree.genus[[1]]
...
sim.fit.full <- mclapply(index, FUN = function(x) run.bisse.bound.mle(tree = tree.genus[[1]]
                                  , st = sim.st[[x]][[1]], unres = sim.st[[x]][[2]]
                                  , low = 0, up = 15
...
emp.fit.full <- run.bisse.bound.mle(tree = tree.genus[[1]], st = state, unres = unres, constrain = "FALSE"
                            , low = 0, up = 15, flag = "Empirical - full")
save(emp.fit.full, file = "emp.fit.full.RData")

emp.fit.cons <- run.bisse.bound.mle(tree = tree.genus[[1]], st = state, unres = unres, constrain = "TRUE"
                            , low = 0, up = 15, flag = "Empirical - cons")
