...
## Plot of the diversification and transition rates for the analysis of coral-mimics
##      versus non-mimics.
## Only five runs here. Need to runs more replicates.
...
post.one.rate <- lapply(1:5, function(x) mcmc.onerate.C[[x]][5000:10000,] )
post.one.rate <- do.call(rbind, post.one.rate)
post.two.rate <- lapply(1:5, function(x) mcmc.tworate.C[[x]][5000:10000,] )
...
## Note that this is net diversification rate.
md.net0.one <- sapply(post.one.rate[3] - post.one.rate[4], median)
md.net0.two <- sapply(post.two.rate[3] - post.two.rate[5], median)
md.net1.two <- sapply(post.two.rate[4] - post.two.rate[6], median)
md.qs.one <- sapply(post.one.rate[5:6], median)
md.qs.two <- sapply(post.two.rate[7:8], median)
...
wd1 <- 0.15
wd2 <- wd1
#wd2 <- 0.1 # This is to be used when xlim = 8. So both graphs would have the same width of the histogram bars.

## Breaks for each parameter:
## First graph:
br.pr.1 <- to.break(a.p, wd1)
br.two.0 <- to.break(post.two.rate[,3] - post.two.rate[,5], wd1)
br.two.1 <- to.break(post.two.rate[,4] - post.two.rate[,6], wd1)
br.one.0 <- to.break(post.one.rate[,3] - post.one.rate[,5], wd1)
## Second graph:
br.pr.2 <- to.break(a.p, wd2)
br.two.q1 <- to.break(post.two.rate[,7], wd2)
br.two.q0 <- to.break(post.two.rate[,8], wd2)
br.one.q1 <- to.break(post.one.rate[,5], wd2)
br.one.q0 <- to.break(post.one.rate[,6], wd2)

pdf(file = "Figure_3_mimics_Caetano_et_al_2016.pdf", width = 14, height = 7)
par(mfrow = c(1,2))

## Make the graph with the net diversification rates (left):

h0 <- hist(a.p, breaks = br.pr.1, plot = FALSE)
h1 <- hist(post.one.rate[,3] - post.one.rate[,4], breaks = br.one.0, plot = FALSE)
h2 <- hist(post.two.rate[,3] - post.two.rate[,5], breaks = br.two.0, plot = FALSE)
h3 <- hist(post.two.rate[,4] - post.two.rate[,6], breaks = br.two.1, plot = FALSE)
...
## The 95% CI lines:
div1 <- quantile(ecdf(post.two.rate[,3] - post.two.rate[,5]), probs = c(0.025, 0.975))
div2 <- quantile(ecdf(post.two.rate[,4] - post.two.rate[,6]), probs = c(0.025, 0.975))
div3 <- quantile(ecdf(post.one.rate[,3] - post.one.rate[,4]), probs = c(0.025, 0.975))
...
## The mean lines:
segments(x0 = md.net0.two, y0 = 0, x1 = md.net0.two, y1 = 0.25, col = "#00000060"
         , lwd = 2.5, lend = 1, lty = 2)
segments(x0 = md.net1.two, y0 = 0, x1 = md.net1.two, y1 = 0.25, col = "#ff000080"
         , lwd = 2.5, lend = 1, lty = 2)
segments(x0 = md.net0.one, y0 = 0, x1 = md.net0.one, y1 = 0.25, col = "#fecd0060"
         , lwd = 2.5, lend = 1, lty = 2)
## Median values in the top of the lines:
tx <- c(md.net0.two, md.net1.two, md.net0.one)
...
legend(x = 6.0, y = 0.8,legend = c("Non-mimic","Coral-mimic","Color independent", "prior")
...
h2 <- hist(post.two.rate[,7], breaks = br.two.q1, plot = FALSE)
h3 <- hist(post.two.rate[,8], breaks = br.two.q0, plot = FALSE)
h4 <- hist(post.one.rate[,5], breaks = br.one.q1, plot = FALSE)
h5 <- hist(post.one.rate[,6], breaks = br.one.q0, plot = FALSE)
...
plot(h1, main = "", xlab = "", ylab = "", col = "#0000ff40", border = "#0000ff60"
   , xlim = c(0,12), ylim = c(0,1), axes = FALSE)
## Light red:
...
tra1 <- quantile(ecdf(post.two.rate[,7]), probs = c(0.025, 0.975))
tra2 <- quantile(ecdf(post.two.rate[,8]), probs = c(0.025, 0.975))
tra3 <- quantile(ecdf(post.one.rate[,5]), probs = c(0.025, 0.975))
tra4 <- quantile(ecdf(post.one.rate[,6]), probs = c(0.025, 0.975))
...
mtext("Probability density", 2, line=2.5, cex = 1.5)
legend(x = 3.8, y = 0.8,legend = c("Non-mimic to coral-mimic (q_01)","Coral-mimic to non-mimicryptic (q_10)", "prior")
...
######################################################################################
## This is actually a figure that will be in the manuscript.
## Bonus figure - Net diversification plot with the contrasting vs. the cryptic categories.
## Note that this figure is not part of the main text. We might include this as the supplementary
##      material.
## Taking out a 50% burn-in and combining the posterior from the independent MCMC runs:
...
wd1 <- 0.15
wd2 <- wd1
## wd2 <- 0.1 ## Same as above, this is when using xlim = 8.
...
pdf(file = "Figure_3_contrasting_Caetano_et_al_2016.pdf", width = 14, height = 7)
par(mfrow = c(1,2))
